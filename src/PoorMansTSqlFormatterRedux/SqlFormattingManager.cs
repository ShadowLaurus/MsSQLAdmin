...
namespace PoorMansTSqlFormatterRedux
{
    public class SqlFormattingManager 
    {
        //default to built-in
        public SqlFormattingManager() : this(new TSqlStandardTokenizer(), new TSqlStandardParser(), new TSqlStandardFormatter()) { }

        //most common use-case, define only formatter
        public SqlFormattingManager(ISqlTreeFormatter formatter) : this(new TSqlStandardTokenizer(), new TSqlStandardParser(), formatter) { }
        ...
        public string Format(string inputSQL, ref bool errorEncountered)
        {
            ...
            errorEncountered = (sqlTree.SelectSingleNode(string.Format("/{0}/@{1}[.=1]", SqlXmlConstants.ENAME_SQL_ROOT, SqlXmlConstants.ANAME_ERRORFOUND)) != null);
